...
<body>
    ...
    <div style="padding: 20px;margin-bottom: 100px;">
        ...
        <label style="font-size: 20px;">流失率数据详情</label><br>
        ...
        <div class="row">
            <div class="col-md-12 table-responsive">
                <table class="table table-hover">
                    ...
                    <c:if test="${error != ''}">
                        <tr>
                            <td colspan="8" style="text-align: center;">${error}</td>
                        </tr>
                    </c:if>
                    <c:forEach items="${flowAwayDailyEntities}" var="zhi">
                        <tr>
                            <td>${zhi.flow_away_time}</td>
                            <td>${zhi.user_id}</td>
                            <td>${zhi.nickname}</td>
                            <td>
                                <c:if test="${zhi.user_source == 1}">默认</c:if>
                                <c:if test="${zhi.user_source == 2}">被推广用户</c:if>
                            </td>
...
<script type="text/javascript">
    ...
    function mofen(page){
        ...
        window.location.href="/flow_away_details?page="+page+"&limit="+y+"&chooseDate="+chooseDate;
    }
    ...
    $(function () {
        $("#detail_search").click(function () {
            ...
            if(chooseDate == ""){
                ...
                window.location.href="/flow_away_details?page=1&limit="+y+"&chooseDate="+chooseDate;
            }
        });
        ...
        $("#limit").change(function () {
            var limit = $(this).val();
            var chooseDate = $("#chooseDate").val();
            window.location.href="/flow_away_details?page=1&limit="+limit+"&chooseDate="+chooseDate;
        });
