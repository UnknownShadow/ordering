...
<body>
    <ul class="layui-nav" lay-filter="">
        ...
        <li class="layui-nav-item">
            ...
            <dl class="layui-nav-child" style="margin-top: -20px;"> <!-- 二级菜单 -->
                <%--<dd style="padding-top: 5px;"><a href="/add_users">付费统计</a></dd>--%>
        ...
        <li class="layui-nav-item">
            ...
            <dl class="layui-nav-child" style="margin-top: -20px;"> <!-- 二级菜单 -->
                <%--<dd style="padding-top: 5px;"><a href="/add_users">流失统计</a></dd>--%>
    ...
    <div style="padding: 20px;margin-bottom: 100px;">
        ...
        <label style="font-size: 20px;">活跃数详情</label><br>
        ...
        <div class="row">
            <div class="col-md-12 table-responsive">
                <table class="table table-hover">
                    ...
                    <c:if test="${error != ''}">
                        <tr>
                            <td colspan="10" style="text-align: center;">${error}</td>
                        </tr>
                    </c:if>
                    <c:forEach items="${realtimeOnlineHistoriesEntities}" var="zhi">
                        <tr>
                            <td>${zhi.created_at}</td>
                            <td>${zhi.user_id}</td>
                            <td>${zhi.nickname}</td>
                            <td>${zhi.total_club_games}</td>
                            <td>${zhi.total_games}</td>
                            <td>
                                <c:if test="${zhi.often_play_games == 2}">上游</c:if>
                                <c:if test="${zhi.often_play_games == 3}">激K</c:if>
                                <c:if test="${zhi.often_play_games == 4}">麻将</c:if>
                            </td>
...
<script type="text/javascript">
    ...
    function mofen(page){
        ...
        window.location.href="/active_details?page="+page+"&limit="+y+"&chooseDate="+chooseDate;
    }
    ...
    $(function () {
        $("#detail_search").click(function () {
            ...
            if(chooseDate == ""){
                ...
                window.location.href="/active_details?page=1&limit="+y+"&chooseDate="+chooseDate;
            }
        });
        ...
        $("#limit").change(function () {
            var limit = $(this).val();
            var chooseDate = $("#chooseDate").val();
            window.location.href="/active_details?page=1&limit="+limit+"&chooseDate="+chooseDate;
        });
